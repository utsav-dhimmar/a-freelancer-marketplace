import { Freelancer, type IFreelancer, type IPortfolioItem } from "../model/freelancer.model.js";
...
export class FreelancerService {
    ...
    async findByUserId(userId: string): Promise<IFreelancer | null> {
        return Freelancer.findOne({ user: userId }).populate("user", "-password -refreshToken");
    }
     ...
     */
    async updateFreelancer(userId: string, data: UpdateFreelancerData): Promise<IFreelancer | null> {
     ...
     */
    async getAllFreelancers(page: number = 1, limit: number = 10): Promise<{
     ...
     */
    async searchBySkills(skills: string[], page: number = 1, limit: number = 10): Promise<{
        freelancers: IFreelancer[];
        total: number;
        page: number;
        totalPages: number;
     ...
     * Add portfolio item
     */
    async addPortfolioItem(userId: string, item: IPortfolioItem): Promise<IFreelancer | null> {
        return Freelancer.findOneAndUpdate(
            { user: userId },
            { $push: { portfolio: item } },
            { new: true }
     ...
     */
    async removePortfolioItem(userId: string, index: number): Promise<IFreelancer | null> {
        const freelancer = await Freelancer.findOne({ user: userId });
     ...
     */
    async updateRating(freelancerId: string, newRating: number): Promise<IFreelancer | null> {
